...
const AddJob = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Use the authenticated endpoint with token in headers
      const response = await axios.post(`${backendUrl}/api/company/post-job`, jobData, {
        headers: {
          'token': companyToken
        }
      })
      ...
      if (error.response?.status === 401 || error.response?.data?.message?.includes('Authorised')) {
    ...
    <div className='container p-4'>
      {!companyToken ? (
        <div className='text-center py-10'>
          <p className='text-gray-600 mb-4'>Please login to add a job</p>
          <button 
